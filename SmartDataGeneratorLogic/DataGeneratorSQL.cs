...
{
	public class DataGeneratorSQL : SmartDataGeneratorLogic.IDataGenerator
	{
		#region Fields
		...
		public event UIDelegate UpdateUI;
		...
		public string Generate()
		{
			...
			if (x)
			{
				...
				InsertData();
				sw.Stop();
				return string.Format("Total time taken : {0:0.00} seconds [{1}ms]\r\nTime taken per row : [{2}ms]", sw.ElapsedMilliseconds / 1000, sw.ElapsedMilliseconds, sw.ElapsedMilliseconds / _TotalRowsToGenerate);
			}
			else
			{
				return string.Format("Tables do not exist. {0}, {1}", _MainTableName, _SubTableName);
			}
		}
		...
		private void GenerateDependencyData(List<string> emptyDependencyTables)
		{
			if (emptyDependencyTables.Count > 0)
			{
				...
				{
					if (d.ToUpper() != _MainTableName.ToUpper())
					{
						var mainTableName = d;
						var subTableName = d.Replace("_HISTORY", "_WORKING");
						var verControlled = d.Contains("_HISTORY");
						var dependencyDataGenerator = new SmartDataGeneratorLogic.DataGeneratorSQL(1, 10, _ConnString, verControlled, mainTableName, subTableName);
					...
					else
					{
						if (UpdateUI != null)
						{
							UpdateUI("Skipping, The dependency table has pig-ear relationship!");
						}
					}
				}
			}
		...
		private bool CheckTableExists()
		{
			...
			using (var conn = new SqlConnection(_ConnString))
			{
				...
				cmd.CommandText = query;
				cmd.Parameters.AddWithValue("@TableName", _MainTableName);
				int.TryParse(cmd.ExecuteScalar().ToString(), out scalar1);
				conn.Close();
				if (_VerControlled)
				{
					conn.Open();
					...
					cmd2.CommandText = query;
					cmd2.Parameters.AddWithValue("@TableName", _SubTableName);
					int.TryParse(cmd2.ExecuteScalar().ToString(), out scalar2);
					conn.Close();
				}
			}
			exists = (scalar1 > 0 && scalar2 > 0 && _VerControlled) || (!_VerControlled && scalar1 > 0);
			return exists;
		}
		...
		private bool GetFromDependencyData(string columnName, out string data)
		{
			...
			foreach(var d in DependencyInfo){
				if (d.ColumnName == columnName)
				{
					...
					data = dataAvailable.Values.Skip(Utility.Random(dataAvailable.Values.Count)).FirstOrDefault();
					if (data != null)
					{
						data = string.Format("'{0}'", data);
					}
		...
		{
			var batchCount = int.Parse(Math.Floor(double.Parse((_TotalRowsToGenerate / BATCH_SIZE).ToString())).ToString());
			var left_over = _TotalRowsToGenerate - batchCount * BATCH_SIZE;
			for (int i = 1; i <= batchCount; i++)
			{
				GenerateRecords(RowNumberStartAt, BATCH_SIZE);
				RowNumberStartAt = RowNumberStartAt + (BATCH_SIZE * i);
				if (UpdateUI != null) { UpdateUI(string.Format("Total of {0} rows created so far.", i * BATCH_SIZE)); }
			}
			if (left_over > 0)
			{
				GenerateRecords(RowNumberStartAt, _TotalRowsToGenerate);
			}
			if (UpdateUI != null) { UpdateUI(string.Format("\r\nDone creating {0} records.", _TotalRowsToGenerate)); }
		}
		...
		private bool GenerateRecords(int start, int end)
		{
			...
			{
				if (UpdateUI != null) { UpdateUI(ex.Message); }
			}
			return result;
		}
	}
